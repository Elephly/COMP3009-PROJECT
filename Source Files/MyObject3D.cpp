...
void MyObject3D::SetPosition(float const & x, float const & y, float const & z)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetPosition(x + ((*it)->GetPosition().GetX() - position.GetX()),
			y + ((*it)->GetPosition().GetY() - position.GetY()),
			z + ((*it)->GetPosition().GetZ() - position.GetZ()));
	}
	position = MyVector3D(x, y, z);
}
...
void MyObject3D::SetPosition(MyVector3D const & vector)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetPosition(vector + ((*it)->GetPosition() - position));
	}
	position = vector;
}
...
void MyObject3D::SetRotation(float const & x, float const & y, float const & z)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetRotation(x + ((*it)->GetRotation().GetX() - rotation.GetX()),
			y + ((*it)->GetRotation().GetY() - rotation.GetY()),
			z + ((*it)->GetRotation().GetZ() - rotation.GetZ()));
	}
	rotation = (x, y, z);
}
...
void MyObject3D::SetRotation(MyVector3D const & vector)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetRotation(vector + ((*it)->GetRotation() - rotation));
	}
	rotation = vector;
}
...
void MyObject3D::SetScale(float const & x, float const & y, float const & z)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetScale(x + ((*it)->GetScale().GetX() - scale.GetX()),
			y + ((*it)->GetScale().GetY() - scale.GetY()),
			z + ((*it)->GetScale().GetZ() - scale.GetZ()));
	}
...
void MyObject3D::SetScale(MyVector3D const & vector)
{
	for (std::vector<MyObject3D *>::iterator it = children->begin(); it != children->end(); ++it)
	{
		(*it)->SetScale(vector + ((*it)->GetScale() - scale));
	}
	scale = vector;
}
